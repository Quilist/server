...
class itemsController {
    ...
    async edit(req, res, next) {
        try {
            await itemsService.edit(req.originalUrl.split("/")[1], { ...req.body }, Number(req.params.id), token);
    ...
    async delete(req, res, next) {
        try {
            await itemsService.delete(req.originalUrl.split("/")[1], Number(req.params.id), token);

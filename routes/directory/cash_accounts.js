...
router.post("/add", utils.isTokenValid, async (req, res) => {
    let { id_type_accounts, name, id_user_currencies, id_bank_name, balanceIn, turnoverDebt, turnoverCred, mono_key, privat24_card, privat24_entity } = req.body;
...
router.post("/:id/edit", utils.isTokenValid, async (req, res) => {
    const { id_type_accounts, name, id_all_currencies, id_bank_name, balanceIn, turnoverDebt, turnoverCred, api_key } = req.body;

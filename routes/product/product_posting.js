...
router.post("/add", async (req, res) => {
  ...
  try {
    //save main
    const productPosting = await prisma.products_posting.create({ data: data })
    ...
    if(products?.length > 0) {
      ...
      products.forEach(function(item) {
        product = {
          ...item,
          id_user: req.token.id,
          products_posting_id: productPosting.id,
          created_at: dateMs,
          updated_at: dateMs,
        };
      ...
      const createProductPromises = productData.map((product) => {
        return prisma.products_posting_product.create({
          data: product
        })
      })
        ...
        const p = await prisma.products_leftover.findUnique({
          where: {
            product_id_storehouse_id: {
              ...
              storehouse_id: productPosting.storehouse_id
            }
          },
        })
        ...
        if(p) {
          ...
          return await prisma.products_leftover.create({
            data: {
              qnt: product.qnt,
              storehouse_id: productPosting.storehouse_id,
              currency_id: productPosting.currency_id,
              product_id: product.product_id,
              price: product.sum,
              created_at: dateMs,
              updated_at: dateMs
...
router.get("/:id", (req, res) => {
  ...
  prisma.products_posting.findUnique({
    where: {
...
router.post("/:id/edit", async (req, res) => {
  ...
  try {
    const productPosting = await prisma.products_posting.update({ data: { ...data }, where: { id: Number(req.params.id) } });
...
router.post("/:id/remove", async (req, res) => {
  const productPosting = await prisma.products_posting.findUnique({
    where: {
      id: Number(req.params.id)
    },
    include: {
  ...
  if(productPosting.products) {
    //--- adjust storehouse balances
    const createProductLeftoverPromises = productPosting.products.map(async (product) => {
      const p = await prisma.products_leftover.findUnique({
        where: {
          product_id_storehouse_id: {
            product_id: product.product_id,
  ...
  await prisma.products_posting.delete({ where: { id: Number(req.params.id) } })
